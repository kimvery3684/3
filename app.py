...
# --- [1. 기본 설정] ---
st.set_page_config(page_title="숫자 퀴즈 생성기 (이미지 전용)", page_icon="🔢", layout="wide")

FONT_FILE = "NanumGothic-ExtraBold.ttf"  # 폰트 파일이 같은 폴더에 있어야 합니다.

# --- [2. 비밀번호 보안 (선택 사항)] ---
# 필요 없다면 이 함수와 호출 부분을 주석 처리하세요.
def check_password():
    ...
    with col2:
        ...
        # Streamlit Cloud 배포 시 Secrets에 APP_PASSWORD 설정, 로컬은 "1234"
...
if not os.path.exists(FONT_FILE):
    st.error(f"🚨 폰트 파일({FONT_FILE})이 없습니다! 한글이 깨져 보일 수 있습니다.")
...
with st.sidebar:
    ...
    with st.expander("1. ⬆️ 상단바 디자인", expanded=True):
        ...
        st.image(create_header_preview(preview_design), caption="상단바 실시간 미리보기", use_column_width=True)
    ...
    with st.expander("2. ⬇️ 하단바 디자인", expanded=True):
        ...
        bot_spacing = col_b2.slider("하단 줄간격", 0, 100, 30)
